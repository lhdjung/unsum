...
To check this, first use `closure_horns_min_max_bar()`. It shows the minimum and maximum possible variability as measured by the horns index, i.e., the average distributions of those samples with the lowest and highest horns index values:

```{r}
closure_horns_min_max_bar(data, frequency = "percent")
```
...
You could also run `closure_horns_histogram()`. It visualizes the distribution of horns values across all samples:
