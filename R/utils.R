...
# Helper for `closure_read()` to check the name of the folder to read from.
# Correct example: "CLOSURE-3_5-1_0-90-1-5-up_or_down-5"
is_output_folder <- function(string) {

  # Check for terminal dash
  if (grepl("-$", string)) {
    return(FALSE)
  }
  ...
  # Check if we have exactly 8 parts (fixed number of values)
  if (length(parts) != 8) {
    return(FALSE)
  }

  # Check for any empty parts
  if (any(parts == "")) {
    return(FALSE)
  }

  # Check that position 4 is a whole number (the "90" location)
  if (!grepl("^[0-9]+$", parts[4])) {
    return(FALSE)
  }

  # Check that positions 5 and 6 are whole numbers (the "1-5" locations)
  if (!grepl("^[0-9]+$", parts[5]) || !grepl("^[0-9]+$", parts[6])) {
    return(FALSE)
  }

  # Check that position 7 is a string (the "very next location")
  # Allow letters, numbers, and underscores for multi-word strings
  if (!grepl("^[a-zA-Z][a-zA-Z0-9_]*$", parts[7])) {
    return(FALSE)
  }

  # Validate that other positions contain reasonable values
  # Positions 2 and 3 should be numbers (whole or decimal with underscores)
  number_pattern <- "^[0-9]+(_[0-9]+)?$"
  if (!grepl(number_pattern, parts[2]) || !grepl(number_pattern, parts[3])) {
    return(FALSE)
  }

  # Position 1 should be a string (like "CLOSURE")
  if (!grepl("^[a-zA-Z][a-zA-Z0-9_]*$", parts[1])) {
    return(FALSE)
  }

  # Position 8 can be flexible (number or string)
  if (!grepl("^[a-zA-Z0-9_]+$", parts[8])) {
    return(FALSE)
  }
