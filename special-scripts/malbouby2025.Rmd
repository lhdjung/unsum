...
```{r}
closure_plot_bar_min_max(data_bmi_horns, frequency = "absolute")
```
...
```{r}
closure_plot_horns_histogram(data_bmi_horns)
```
...
```{r}
closure_plot_bar_min_max(data_age_horns, frequency = "absolute", text_size = 10)
closure_plot_horns_histogram(data_age_horns)
```
